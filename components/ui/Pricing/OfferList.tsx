const checkIcon = (
  <svg width="10" height="10" viewBox="0 0 8 6" className="fill-current">
...
const OfferList = ({
  ...
  return (
    <div className="mb-3 flex items-center">
      <span className={`mr-3 flex h-[18px] w-full max-w-[18px] items-center justify-center rounded-full ${status ? 'bg-primary ' : 'bg-red-500'} bg-opacity-10 ${status ? 'text-primary' : 'text-red-500'}`}>
        {status ? checkIcon : crossIcon}
      </span>
      <p className="m-0 text-base text-body-color">{text}</p>
    </div>
  );
};
